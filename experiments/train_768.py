...
def train_768(data_root, model_path, save_dir, epochs=20, batch_size=1, lr=1e-5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"768x768 egitim | lr={lr} | batch={batch_size} | Cihaz: {device}")

    train_loader, val_loader = get_dataloaders(data_root, img_size=768, batch_size=batch_size)
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append({"epoch": epoch, "train_iou": t_iou/n, "val_iou": v_iou/nv})
    ...
    with open(os.path.join(save_dir, "history_768.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    train_768(DATA_ROOT, MODEL_PATH, SAVE_DIR, epochs=20, lr=1e-5)
